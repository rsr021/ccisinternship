		...
		<div class="table-responsive">
			<table id="example" class="table table-sm" style="width:100%">
				...
				$rs = mysqli_query($db_connection,'SELECT facultyid, username, firstname, middlename,
						lastname, email, sectionid, courseid FROM tblfaculty WHERE facultyid!=1
						ORDER BY firstname');
				while($rw=mysqli_fetch_array($rs)){
					echo'<tr>
						...
						if($rw['sectionid']){
							echo SectionCode($rw['sectionid']);
						} else {
							echo'<span style="color:red;">NO SEC</span>';
						}
						
						
						$link = "TINY.box.show({url:'pages/getsection.php?facultyid=".$rw['facultyid']."',width:600,height:90 })";
//<td><a href="javavascript:void();" onclick="'.$link.'">Assign</a></td>
//<td><a href="javascript:void();">Send Message</a></td>
						echo'</td>
