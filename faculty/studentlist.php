...
<?
$jscript = 'loadPage(\'faculty/tmpliststudentlist.php?companyid=\'+object(\'companyid\').value\'
													&str=\'+object(\'str\').value,\'show\')';
?>
<h5><?=CourseName($_SESSION['courseid'])?>&nbsp;&nbsp;(<?=SectionCode($_SESSION['sectionid'])?>)</h5>
<div class="container mb-4 d-flex">
	<? echo '<select onchange="loadSubContent(\'faculty/tmpliststudentlist.php?str=\'+object(\'str\').value+\'&companyid=\'+object(\'companyid\').value,\'show\')" class="form-control" id="companyid");">';
...
echo '</select>&nbsp;'; ?>
	<input type="text" onkeyup="loadSubContent('faculty/tmpliststudentlist.php?str='+this.value+'&companyid='+object('companyid').value,'show')" class="form-control" id="str" placeholder="Search Student...">
	<button type="button" onclick="loadSubContent('faculty/tmpliststudentlist.php?str='+this.value+'&companyid='+object('companyid').value,'show')" style="background-color: #9e1e1e;" class="btn text-light">Search</button>
</div>

<div id="show">
<?include('tmpliststudentlist.php');?>
</div>
