...
$otp_student = GetValue('SELECT token FROM tblreset_f WHERE token=\''.$otp.'\' AND is_inactive=0');
...
<a class="button" href="javascript:void();" onclick="save_password(\''.$email.'\')">Submit</a>';
  mysqli_query($db_connection,'UPDATE tblreset_f SET is_inactive=1 WHERE token=\''.$otp.'\' ');
} else {
