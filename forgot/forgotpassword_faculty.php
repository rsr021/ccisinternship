...
.button2{
	...
	function email_authenticate(){
		...
		if(email!=''){
			loadPage('pages_f/emailverify.php?email='+email,'content_f')
		} else {
			alert('Please input your email.');
		}
	}
	function send_opt(email){
		loadPage('../email/forgot_f.php?email='+email,'showy');
	...
	function match_otp(email){
		...
		if(otp==''){
			...
			loadPage('pages_f/newpassword.php?otp='+otp+'&email='+email,'content_f');
		}
	}
	function save_password(email){
		...
		if (newp==''){
			...
			loadPage('pages_f/success.php?newp='+newp+'&email='+email,'content_f');
		}
