...
if (file_exists('../includes/database.php')) { include_once('../includes/database.php'); }
	...
	if(isset($_GET['description'])){
		...
		$rs2 = mysqli_query($db_connection,'select * from '.$_SESSION['tmp_dtr']);
		while ($rw2=mysqli_fetch_array($rs2)) {
			mysqli_query($db_connection, 'INSERT INTO tblaccomplishmentreport_dtr SET 
						reportid='.$reportid.',day=\''.$rw2['day'].'\',timein=\''.$rw2['timein'].'\',
						timeout=\''.$rw2['timeout'].'\',totalhours='.$rw2['totalhours'].' ') 
						or die(mysqli_error($db_connection));
		}
...
if($is_ojt){
    ...
    echo '<table border="1" style="border-collapse:collapse;">
            ...
            $q = 'SELECT a.*,SUM(b.totalhours) as tot
						FROM tblaccomplishmentreport a, tblaccomplishmentreport_dtr b
						WHERE a.reportid=b.reportid AND a.studentid='.$_SESSION['studentid'].' GROUP BY a.reportid';
			//echo $q;
